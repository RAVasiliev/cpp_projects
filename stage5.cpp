...
bool func_0_param(char *str)
{
	return ((str_cmp(str, "?my_id"))||(str_cmp(str, "?turn"))||
		(str_cmp(str, "?players"))||(str_cmp(str, "?have_winner"))||
...
{
	if (str_cmp(name, "?have_winner"))
		return (all_info -> info_players -> have_winner());
...
{
	if (str_cmp(name, "?production"))
		return (all_info -> info_players -> get_prod(
...
public:
			...
			Work_With_Tables *tab, game_info *all_info) const
	{
		...
		if (res1 == NULL)
		{
			printf ("Oooops.. Didn't know who is %s\n", i1->Get());
			throw UNDEF_VAR;
		}
		int res = *res1;;
		delete operand1;
...
public:
			...
			Work_With_Tables *tab, game_info *all_info) const
	{
		RPNElem *operand1 = Pop(stack);
		RPNString *i1 = dynamic_cast<RPNString*>(operand1);
		if (str_cmp(i1->Get(), "endturn"))
		{
			all_info->mark->month_new();	
			turn(all_info ->com,all_info -> inp,all_info->info_op,
				all_info -> mark, all_info -> info_players);
		}
		delete operand1;
		return NULL;
	}
...
public:
			...
			Work_With_Tables *tab, game_info *all_info) const
	{
		...
		i2->Get();
		i1->Get();
		if (str_cmp(i1->Get(), "prod"))
			prod(all_info -> com, all_info -> inp, i2->Get());
		delete operand1;
		delete operand2;
		return NULL;
	}
...
public:
			...
			Work_With_Tables *tab, game_info *all_info) const
	{
		...
		i3->Get();
		if (str_cmp(i1->Get(), "buy"))
			buy(all_info -> com, all_info -> inp,
				i3->Get(), i2->Get());	
		if (str_cmp(i1->Get(), "sell"))
			sell(all_info -> com, all_info -> inp,
				i3->Get(), i2->Get());	
...
void winner_test(game_info *inf)
{
	if (str_cmp(inf -> info_players -> get_winner(),
			inf -> info_players -> get_myname()))
		printf("I'm a winner!\n");
	else
		printf("Winner is %s!\n", inf -> info_players -> get_winner());
}
